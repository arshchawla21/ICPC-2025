...
// priority_queue<int, vector<int>, MyCustomComparator> pq;
...
void solve() {
  ...
  vector<int> deals(21);
  vector<ll> precomp(21); // precomp[i] is the price to get 3^i watermelons
  for (int x = 0; x <= 20; ++x) {
    precomp[x] = ((ll) pow(3,x+1) + (ll) x * (ll) pow(3,x-1));
  }
  ...
  for (int x = 20; x > 0; x--) {
    ...
    if (deals[x] > 0) {
      // end here
      if (deals[x] > extra_deals) {
        deals[x-1] += 3 * extra_deals;
        cost = cost - extra_deals * precomp[x] + 3 * extra_deals * precomp[x-1];
        break;
      } else {
        extra_deals = extra_deals - deals[x];
        deals[x-1] += 3 * deals[x];
        cost = cost - deals[x] * precomp[x] + 3 * deals[x] * precomp[x-1];
        deals[x] = 0;
      }
    }
  }
